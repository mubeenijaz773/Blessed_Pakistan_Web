...
<button className={`${style.button} float-right`} onClick={handleButtonClick} >
...
const Card=({data})=>{
  ...
  return(
    ...
    <div className="w-full py-3 mt-5 flex gap-2 flex-col  font-sans rounded  px-2 bg-gray-100">
        ...
        <button     onClick={toggleDialog} type="button" className="mt-7 flex justify-center gap-2 w-full font-sans text-white bg-gradient-to-r from-red-400 via-red-500
...
<CallDialog call={data?.mobile} isOpen={isDialogOpen} onClose={toggleDialog} />
  
<EmailDailogBox item={data}  isOpen={isEmailDialogOpen} onClose={closeEmailDialog}  />
              <button onClick={openEmailDialog}  type="button" className="mt-5 flex justify-center gap-2 w-full font-sans text-white bg-gradient-to-r from-red-400 via-red-500
           to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800
            font-medium rounded-lg text-sm  py-2.5 text-center mr-2 mb-2">
